...
@section('title', 'Edit Pemesanan #' . $booking->id)
...
<div class="max-w-2xl mx-auto">
    ...
    <div class="mb-6 flex items-center gap-2 text-sm text-gray-500">
        ...
        <a href="{{ route('bookings.show', $booking) }}" class="hover:text-indigo-600 transition-colors">
            #{{ $booking->id }}
        </a>
        <svg class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor">
            <path stroke-linecap="round" stroke-linejoin="round" d="m8.25 4.5 7.5 7.5-7.5 7.5" />
    ...
    <div class="bg-white rounded-2xl shadow px-8 py-8">
        <h1 class="text-xl font-bold text-gray-800 mb-6">Edit Pemesanan #{{ $booking->id }}</h1>

        <form method="POST" action="{{ route('bookings.update', $booking) }}" novalidate>
            @csrf
            @method('PUT')
            ...
            <div class="mb-5">
                ...
                >
                    <option value="" disabled>— Pilih Approver L1 —</option>
                    @foreach ($approvers as $approver)
                        <option value="{{ $approver->id }}"
                            {{ old('approver_level_1_id', $booking->approver_level_1_id) == $approver->id ? 'selected' : '' }}>
                            {{ $approver->name }}
                        </option>
                    @endforeach
                </select>
            ...
            <div class="mb-5">
                ...
                >
                    <option value="" disabled>— Pilih Approver L2 —</option>
                    @foreach ($approvers as $approver)
                        <option value="{{ $approver->id }}"
                            {{ old('approver_level_2_id', $booking->approver_level_2_id) == $approver->id ? 'selected' : '' }}>
                            {{ $approver->name }}
                        </option>
                    @endforeach
                </select>
            ...
            <div class="mb-5">
                ...
                <input
                    ...
                    id="keperluan"
                    name="keperluan"
                    value="{{ old('keperluan', $booking->keperluan) }}"
                    maxlength="255"
                    class="w-full rounded-lg border px-4 py-2.5 text-sm shadow-sm focus:outline-none focus:ring-2 focus:ring-indigo-500
            ...
            <div class="mb-8">
                ...
                <input
                    ...
                    id="konsumsi_bbm"
                    name="konsumsi_bbm"
                    value="{{ old('konsumsi_bbm', $booking->konsumsi_bbm) }}"
                    min="0"
                    step="0.01"
                    placeholder="0.00"
                    class="w-full rounded-lg border px-4 py-2.5 text-sm shadow-sm focus:outline-none focus:ring-2 focus:ring-indigo-500
